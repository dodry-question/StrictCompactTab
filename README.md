...
* **Color Themes**: Support for Dark, Light, and Nord (Arctic) color schemes. Enhancements like high-contrast text shadows (glow outlining for light theme) ensure elements remain fully readable on custom background images.
...
* **Confidential Weather Widget**: A private weather display located in the top-right corner of the page. It requires manual city input to prevent geo-tracking or IP leakage. Features automatic duplicate city resolution feedback (showing region and country details). If disabled, the widget is completely inactive and initiates zero network requests.
